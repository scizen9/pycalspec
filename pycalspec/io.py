...
_DATASOURCE = os.getenv('CALSPECPATH',
...
CURRENT_CALSPEC_ARCHIVE_URL = "https://archive.stsci.edu/hlsps/" \
                              "reference-atlases/cdbs/current_calspec"
CALSPEC_ARCHIVE_URL = "https://archive.stsci.edu/hlsps/" \
...
def std_radec(stdname):
    ...
    if stdname not in caldata.keys():
        ...
        raise ValueError("Unknown standard '%s' "
                         "[be careful. Case sensitive]" % stdname)
...
def calspec_data():
    ...
    return {str(ll[_name[0]:_name[1]]).replace(" ", "").upper(): {
        "ra": ":".join(str(ll[_ra[0]:_ra[1]]).replace("\t", "").split()),
        "dec": ":".join(str(ll[_dec[0]:_dec[1]]).replace("\t", "").split())
        }
          for ll in calfile[1:]}
...
def calspec_file(stdname, download=True):
    ...
    ----------
    stdname: string
        name of the standard star. Caution: case sensitive.
    download: bool
        download from calspec website if not in local cache
    ...
    if len(specfile) == 0:
        if download:
            ...
            raise IOError("Currently no spectra for %s. \n" % stdname +
                          "Look for it from http://www.stsci.edu/ftp/cdbs/"
                          "current_calspec/ and save in at %s" % _DATASOURCE)
...
def download_calspec(stdname, outdir=None, warn=True):
    ...
    (see get_list_of_calspec_files()) and download the one corresponding to
     stdname The files are saved under the ./data/calspec/ directory except if
     said otherwise.
    ...
    warn: bool
        report where file is downloaded if True
    ...
    files_to_dl = [f_ for f_ in list_of_calspec_files
    ...
    if len(files_to_dl) == 0:
        raise ValueError(f"no calspec file corresponding to {stdname} found in "
                         f"calspec_current or calspec.")
...
def download_list_of_calspec_files(use_current=True, store=True):
    ...
    list_of_files = [ll.split("</a>")[0].split(">")[-1]
                     for ll in calspeclit.text.splitlines() if ".fits" in ll]
    if store:
